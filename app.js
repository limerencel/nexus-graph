(() => {
  ...
  function tick() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (d < min) {
          const push = ((min - d) / d) * COLLIDE * 0.5;
          const px = dx * push;
          const py = dy * push;
          a.x -= px;
          a.y -= py;
  ...
  function fit(pad) {
    ...
    const k = Math.min((w - pad * 2) / bw, (h - pad * 2) / bh, 1.8);
    cam.k = Math.max(0.2, k);
  ...
  function select(n) {
    ...
    $('insp-meta').innerHTML = '';
    const tags = [n.type, ...(n.tags || []), n.degree + ' links'];
  ...
  function drawGrid() {
    ...
    for (let x = x0; x < p1.x; x += step) {
      ctx.moveTo(x, p0.y);
      ctx.lineTo(x, p1.y);
    }
    for (let y = y0; y < p1.y; y += step) {
      ctx.moveTo(p0.x, y);
      ctx.lineTo(p1.x, y);
    }
    ctx.stroke();
  ...
  function draw() {
    ...
    for (const n of nodes) {
      ...
      const on = n === selected || n === hovered || match;
      const color = t ? t.color : '#e8ecf4';

      if (on && !fade) {
        ctx.beginPath();
        ctx.arc(n.x, n.y, n.r + 8, 0, Math.PI * 2);
        ctx.fillStyle = (t ? t.color : '#5ce1e6') + '22';
        ctx.fill();
      }
  ...
  canvas.addEventListener('pointermove', (e) => {
    ...
    if (pointers.size === 2 && pinch) {
      ...
      zoomAt(pinch.mx, pinch.my, d / (pinch.d || 1) / (cam.k / pinch.k));
      cam.x += mx - pinch.mx;
      cam.y += my - pinch.my;
      pinch.mx = mx;
      pinch.my = my;
